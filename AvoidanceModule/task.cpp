...
Task create_task(const std::string& task_filename){
    ...
    if (task_file.is_open())
    {
        ...
        Ship task_ship = Ship(Vector(x, y, 0), Vector(vx, vy, 0), rad, (ModelType)type, id, max_speed, radar_rad);
        task_file >> x >> y;
        Vector task_target{x, y, 0};
        ...
        while (task_file >> x >> y >> vx >> vy >> rad >> type >> id)
        {
            ...
            Obstacle task_obst(Vector(x, y, 0), Vector(vx, vy, 0), rad, (ModelType)type, id);
